...
def setUpAnnualAndQuarterlyObjects():
    ...
    for ticker in tickers:
        print(ticker)
        annualBSObj = readAnnualBSDataWithYFinance(ticker)
        annualBSObjs.append(annualBSObj)
        quarterlyBSObj = readQuarterlyBSDataWithYFinance(ticker)
        quarterlyBSObjs.append(quarterlyBSObj)

        print('FINISHED BALANCE SHEET')

        annualISObj = readAnnualISDataWithYFinance(ticker)
        annualISObjs.append(annualISObj)
        quarterlyISObj = readQuarterlyISDataWithYFinance(ticker)
        quarterlyISObjs.append(quarterlyISObj)

        print('FINISHED INCOME SHEET')

        annualCFObj = readAnnualCFDataWithYFinance(ticker)
        annualCFObjs.append(annualCFObj)
        quartleryCFObj = readQuarterlyCFDataWithYFinance(ticker)
        quarterlyCFObjs.append(quartleryCFObj)

        print('FINISHED CASH FLOW SHEET')

        stockObj = readCommonStockDataWithYFinance(ticker)
        stockObjs.append(stockObj)

        futureStockObj = readFutureStockDataWithYFinance(ticker)
...
def createQuarterlyStockTxtFile(ticker, base_dir):
    ...
    with open(txt_path, "w") as f:
        ...
        f.write("\n"f"EV/EBITDA                                 {annualBSObj.ev_ebitda[0]}")
